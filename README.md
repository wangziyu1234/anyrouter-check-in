...
如果你只需要单账号签到，也可以不配置 `ANYROUTER_ACCOUNTS`，直接添加下面两个 environment secret：

- `ANYROUTER_COOKIE`：浏览器请求头里的整段 Cookie
- `ANYROUTER_NEW_API_USER`：请求头里的 `new-api-user` 值

脚本会自动把这两个值转换成单账号配置。
该模式会直接使用你提供的 Cookie 签到，并跳过浏览器登录/浏览器安装步骤。
