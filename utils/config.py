...
def load_accounts_config() -> list[AccountConfig] | None:
	...
	if not accounts_str:
		...
		api_user = os.getenv('ANYROUTER_NEW_API_USER', '').strip() or os.getenv('ANYROUTER_API_USER', '').strip()
		if cookie and api_user:
			print('[INFO] ANYROUTER_ACCOUNTS not found, using ANYROUTER_COOKIE + ANYROUTER_NEW_API_USER')
			return [
				AccountConfig(
					cookies=cookie,
					api_user=api_user,
